...
class QuadTree:
    ...
    def query_circle(self, boundary, centre, radius, found_payloads):
        """Find the payloads in the quadtree that lie within radius of centre.
        ...
        for point in self.points:
            if (boundary.contains(point) and
                    point.distance_squared_to(centre) <= radius**2):
                # return the payload because that is what we're interested in
                found_payloads.append(point.payload)

        # Recurse the search into this node's children.
        if self.divided:
            self.nw.query_circle(boundary, centre, radius, found_payloads)
            self.ne.query_circle(boundary, centre, radius, found_payloads)
            self.se.query_circle(boundary, centre, radius, found_payloads)
            self.sw.query_circle(boundary, centre, radius, found_payloads)
        return found_payloads

    def query_radius(self, centre, radius, found_payloads):
        ...
        return self.query_circle(boundary, centre, radius, found_payloads)
